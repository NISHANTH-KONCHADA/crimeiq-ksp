...
const randCoord = (base) => +(base + (Math.random() - 0.5) * 0.1).toFixed(4); // Tighter coordinate spread for city accuracy
...
module.exports = async (context, basicIO) => {
  ...
  // We use batch number to create a deterministic offset for exact uniqueness
  ...
  for (let i = start; i < end; i++) {
    // Deterministic unique offsets for this FIR
    const accusedStartIndex = i * 2; // Reserve 2 accused per FIR theoretically
    const victimIndex = i;
    ...
    try {
      ...
      // 40% chance of having 2 accused, 60% chance of 1 accused.
      // We still use deterministic IDs based on accusedStartIndex
      const numAccused = (i % 5 === 0 || i % 7 === 0) ? 2 : 1;
      const localIds = [];
      
      for (let a = 0; a < numAccused; a++) {
        const uniqueAccusedIndex = accusedStartIndex + a;
        const gender = uniqueAccusedIndex % 3 === 0 ? 'Female' : 'Male'; // mostly male for accused distribution logic
      ...
      await victimTable.insertRow({
        fir_id: firId,
        name: getUniqueName(victimIndex + 5000, vGender), // Offset massively to avoid collision with accused names
  ...
  basicIO.write(`Batch ${batch} completed strictly. Inserted ${inserted}/20 FIRs (Rows ${start} to ${end-1}). No duplicates generated.`);
  context.close();
};